    ...
    public Bitmap getCroppedRoundBitmap(Bitmap bmp, int radius) {
        ...
        if (getScaleType() == ScaleType.CENTER_INSIDE) {
            ...
            if (bmpHeight > bmpWidth) {// 高大于宽
                squareWidth = squareHeight = (int) (bmpHeight * Math.sqrt(2.0d));
                y = (squareHeight - bmpHeight) / 2;
                x = (squareHeight - bmpWidth) / 2;
                // 截取正方形图片
                squareBitmap = Bitmap.createBitmap(squareWidth, squareHeight, Config.ARGB_8888);
                Paint paint = new Paint();
                paint.setStyle(Paint.Style.FILL);
                paint.setAntiAlias(true);
                // 设置画笔的颜色
                paint.setColor(Color.WHITE);
                RectF re = new RectF(0, 0, squareWidth, squareHeight);
                Canvas canvas = new Canvas(squareBitmap);//bitmap提供了画布，只在此提供了大小尺寸，偏移后并未有背景显示出来
                canvas.drawRect(re, paint);
                canvas.drawBitmap(bmp, x, y, paint);//绘制的图片和之前的一模一样

            } else if (bmpHeight < bmpWidth) {// 宽大于高
                squareWidth = squareHeight = (int) (bmpWidth * Math.sqrt(2.0d));
                y = (squareWidth - bmpHeight) / 2;
                x = (squareWidth - bmpWidth) / 2;
                squareBitmap = Bitmap.createBitmap(squareWidth, squareHeight, Config.ARGB_8888);
                Paint paint = new Paint();
                paint.setStyle(Paint.Style.FILL);
                paint.setAntiAlias(true);
