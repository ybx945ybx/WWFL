...
import com.haitao.R;
import com.haitao.adapter.MyInviteAdapter;
...
public class MyInviteFragment extends BaseFragment {
    ...
    private int                      mPage;
    private List<InvitedFriendModel> mList;
    ...
    private void initVars() {
        ...
        mPage = 1;
        mList = new ArrayList<>();
        mAdapter = new MyInviteAdapter(mContext, mList);
    }
    ...
    private void loadData() {
        ForumApi.getInstance().userInvitedFriendsListGet(String.valueOf(mPage), String.valueOf(PageConstant.pageSize),
                response -> {
                    if (mMsv == null)
                        return;
                    mMsv.showContent();
                    ...
                    if (TextUtils.equals(response.getCode(), "0")) {
                        InvitedFriendsListModelData data = response.getData();
                        if (data != null) {
                            if (1 == mPage) {
                                mList.clear();
                            }
