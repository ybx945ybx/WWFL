...
import com.haitao.R;
import com.haitao.adapter.BoardSelectPagerAdapter;
...
public class BoardSelectActivity extends BaseActivity {
    ...
    private BoardSelectPagerAdapter      adapter;
    ArrayList<BoardSelectFragment> fragments = null;

    private String selectBoardId = "";

    ViewGroup llProgress_common_progress;
    ...
    private void initData() {
        ...
        fragments = new ArrayList<BoardSelectFragment>();
        for (int i = 0; i < navList.size(); i++) {
            BoardSelectFragment fragment = new BoardSelectFragment();
            Bundle              bundle   = new Bundle();
            bundle.putSerializable(TransConstant.VALUE, navList.get(i).getSectionId());
            bundle.putSerializable(TransConstant.ID, selectBoardId);
            fragment.setArguments(bundle);
            fragments.add(fragment);
        }
        adapter = new BoardSelectPagerAdapter(getSupportFragmentManager(), fragments, navList);
    ...
    public static void launch(Context context) {
        Intent intent = new Intent(context, BoardSelectActivity.class);
